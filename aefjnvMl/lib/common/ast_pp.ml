...
let rec expression_pp fmt expr =
  ...
  | Exp_tuple exprs ->
    fprintf
      ...
      exprs
  | Exp_function (pat, e) -> fprintf fmt "fun %a -> %a" pattern_pp pat expression_pp e
  ...
  | Exp_apply (e1, e2) -> fprintf fmt "(%a %a)" expression_pp e1 expression_pp e2
...
and value_binding_pp fmt vb =
  fprintf fmt "%a = %a" pattern_pp vb.vb_pat expression_pp vb.vb_expr
;;
